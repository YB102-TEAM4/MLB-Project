...
def main(input_file, user, password, host, port, table, database):
    #main(input_file, user, password, host, table, database):
    print "Importing `%s' into MySQL database `%s.%s'" % (input_file, database, table)
    db = MySQLdb.connect(host=host, user=user, port=port ,passwd=password)#?????????????????????????????????????????
    #MySQLdb.connect(host=host, user=user, port=port ,passwd=password)
    cursor = db.cursor()
