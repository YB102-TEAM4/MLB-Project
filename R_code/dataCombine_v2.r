...
for(i in dir.list){
  ...
  for(k in 1:(m-3)){
    ...
    temp[m+k,"p_IR"]=mean(temp[k:k+2,"p_IR"],na.rm=T) # average p_IR  >> 換投時，未被解決的壘上跑壘者數
    temp[m+k,"p_IS"]=mean(temp[k:k+2,"p_IS"],na.rm=T) # average p_IS  >> 換投時，未被解決的壘上跑壘者數中最後有得分的人數
