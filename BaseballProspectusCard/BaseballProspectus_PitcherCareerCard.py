#-*- coding: utf-8 -*-
#所有打者的生涯成績 爬蟲
#從這個網站的球員姓名  http://www.baseballprospectus.com/sortable/index.php?cid=1754950
...
trList=soup.select('#TTdata tr')[5210:]#1 ********************************************
...
for tr in trList:
    ...
    if urlReal==None:#將沒爬到的網站寫進 urlError.txt        
        fod=open('urlPitcherError.txt','a')
        fod.write(url+'\n')
        fod.close()
    ...
    try:
        ...
        fod=open('urlPitcherError.txt','a')#將有問題網址寫入檔案
        fod.write(url+'\n')#.decode('utf8')
        fod.close()
    finally:
    #第四步 善後
